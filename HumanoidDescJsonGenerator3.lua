...
local function getJsonFromDescription(desc)
	...
	-- Daftar kategori property yang ada di HumanoidDescription
	-- Kita masukkan semua kategori agar benar-benar fleksibel
	local propertyGroups = {
		-- Colors
		"HeadColor", "TorsoColor", "LeftArmColor", "RightArmColor", "LeftLegColor", "RightLegColor",
		-- Classic Clothing & Face
		"Shirt", "Pants", "GraphicTShirt", "Face", "Head", "Torso", "LeftArm", "RightArm", "LeftLeg", "RightLeg",
		-- All Accessory Types (Termasuk Layered Clothing 3D)
		"HatAccessory", "HairAccessory", "FaceAccessory", "NeckAccessory", "ShoulderAccessory", 
		"FrontAccessory", "BackAccessory", "WaistAccessory", "ShirtAccessory", "PantsAccessory", 
		"JacketAccessory", "SweaterAccessory", "ShortsAccessory", "DressSkirtAccessory"
	}

	-- Iterasi yang aman
	for _, propName in ipairs(propertyGroups) do
		-- Gunakan pcall supaya kalau Roblox ganti nama property, script gak crash
		local success, val = pcall(function() return desc[propName] end)

		if success and val ~= nil then
			if typeof(val) == "Color3" then
				-- Convert warna ke tabel {R, G, B}
				data[propName] = {math.floor(val.r*255), math.floor(val.g*255), math.floor(val.b*255)}
			elseif typeof(val) == "number" and val ~= 0 then
				data[propName] = val
			elseif typeof(val) == "string" and val ~= "" and val ~= "0" then
				data[propName] = val
			end
		end
	end
